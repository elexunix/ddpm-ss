...
# adopted from https://github.com/Deemp233333/torch_hearing_perception_metrics/blob/main/metrics.py:

def wss(clean_speech: torch.Tensor, processed_speech: torch.Tensor, sample_rate):
  ...
  cent_freq = torch.Tensor([50.0000, 120.000, 190.000, 260.000, 330.000, 400.000, 470.000,
  ...
  bandwidth = torch.Tensor([70.0000, 70.0000, 70.0000, 70.0000, 70.0000, 70.0000, 70.0000,
  ...
  for frame_count in range(num_frames):
    ...
    # if USE_FFT_SPECTRUM:
    clean_spec = torch.square(torch.abs(torch.fft.fft(clean_frame, n_fft)))
    processed_spec = torch.square(torch.abs(torch.fft.fft(processed_frame, n_fft)))
...
def lpcoeff(speech_frame: torch.Tensor, model_order):
    ...
    acorr = R
    refcoeff = rcoeff
    lpparams = torch.concatenate((torch.Tensor([1], device=device), -a))
...
def snr(clean_speech, processed_speech, sample_rate):
    ...
    MIN_SNR = torch.Tensor([-10], device=device)  # minimum SNR in dB
    MAX_SNR = torch.Tensor([35], device=device)  # maximum SNR in dB
    ...
    EPS = torch.Tensor([np.finfo(np.float64).eps], device=device)
    for frame_count in range(num_frames):
        # (1) Get the Frames for the test and reference speech. Multiply by Hanning Window.
        clean_frame = clean_speech[start:start + winlength]
        processed_frame = processed_speech[start:start + winlength]
...
@torch.no_grad()
def hearing_perception_metrics(ests, refs, Fs):
  assert ests.ndim == 2 and refs.ndim == 2 and Fs == 16000
  pesq_values, csig_values, cbak_values, covl_values = [], [], [], []
  # ref: ground truth waveforms, Type: 2D-Tensor
  # est: estimated waveforms, Type: 2D-Tensor
  for est, ref in zip(ests, refs):
    alpha = 0.95
    ...
    pesq_values.append(PESQ.item())
    csig_values.append(CSIG)
    cbak_values.append(CBAK.item())
    covl_values.append(COVL)
  return {
    'pesq': torch.tensor(pesq_values),
    'csig': torch.tensor(csig_values),
    'cbak': torch.tensor(cbak_values),
    'covl': torch.tensor(covl_values),
  }
