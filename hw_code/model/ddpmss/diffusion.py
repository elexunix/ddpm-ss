...
class DiffusionModel(nn.Module):
  ...
  def __init__(self):
    super().__init__()
    self.model = "don't run me please"

  def forward(self, x):
    print('x', x.shape)
    audio = x
    mel = mel_spectogram(sample_rate=22050, hop_length=256, win_length=1024, n_fft=1024, n_mels=80, f_min=0, f_max=8000, power=1.0, normalized=False, norm="slaney", mel_scale="slaney", compression=True, audio=audio).to(device)
    waveforms = diffwave.decode_batch(mel, hop_len=256, fast_sampling=True, fast_sampling_noise_schedule=[0.0001, 0.001, 0.01, 0.05, 0.2, 0.5])
    print('w', waveforms.shape)
    with torch.no_grad():
      return waveforms
