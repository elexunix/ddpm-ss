...
from pipeline import text_encoder as text_encoder_module
from pipeline.base.base_text_encoder import BaseTextEncoder
from pipeline.logger import setup_logging
from pipeline.text_encoder import CTCCharTextEncoder
from pipeline.utils import read_json, write_json, ROOT_PATH
...
class ConfigParser:
  ...
  def get_default_configs(cls):
    config_path = ROOT_PATH / "pipeline" / "config.json"
    with config_path.open() as f:
      return cls(json.load(f))

  @classmethod
  def get_test_configs(cls):
    config_path = ROOT_PATH / "pipeline" / "tests" / "config.json"
    with config_path.open() as f:
      return cls(json.load(f))
