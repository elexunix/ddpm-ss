...
import pipeline.augmentations
import pipeline.datasets
from pipeline import batch_sampler as batch_sampler_module
from pipeline.base.base_text_encoder import BaseTextEncoder
from pipeline.collate_fn.collate import collate_fn
...
def get_dataloaders(configs: ConfigParser):
  ...
  for split, params in configs["data"].items():
    ...
    if split == 'train':
      wave_augs, spec_augs = pipeline.augmentations.from_configs(configs)
      drop_last = True
    else:
    ...
    for ds in params["datasets"]:
      datasets.append(configs.init_obj(
        ds, pipeline.datasets, config_parser=configs,
