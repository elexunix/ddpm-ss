...
device = 'cuda'
#diffwave = DiffWaveVocoder.from_hparams(source="speechbrain/tts-diffwave-ljspeech", savedir="pipeline/model/ddpmss/tmpdir", run_opts={"device": device})
...
class DiffusionModel(nn.Module):
  ...
  def __init__(self):
    super().__init__()
    self.diffwave = DiffWaveVocoder.from_hparams(source="speechbrain/tts-diffwave-ljspeech", savedir="pipeline/model/ddpmss/tmpdir", run_opts={"device": device})
  ...
  def forward(self, x):
    ...
    assert mel.ndim == 4
    mel = mel[:, :, :, :-1]
    #print(f'{x.shape=}, {mel.shape=}')
    #torchaudio.save('diffusion-input.wav', x[0].cpu(), 22050)
    x = torch.stack([
    ...
    #torchaudio.save('diffusion-output.wav', x[0].cpu(), 22050)
    #print('saved, hanging')
    #while True: pass
    return x
