...
if [ ! -d metadata/Libri0Mix ]; then
  echo GENERATING METADATA FOR 0 SPEAKERS
  python3 scripts/create_librimix_metadata.py --librispeech_dir $storage_dir/LibriSpeech --librispeech_md_dir metadata/LibriSpeech --wham_dir $storage_dir/wham_noise --wham_md_dir metadata/Wham_noise --metadata metadata/Libri0Mix --n_src 0
  echo TRUNCATING TABLES
  sed -i '10002,$d' metadata/Libri0Mix/*
fi
